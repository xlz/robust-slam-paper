...
\end{equation}
where, \cj{does $I$ refer to the matrix identity? define}
\begin{equation*}
\begin{aligned}
...
\end{equation}
where, \cj{What is the difference between $H_k^{i_k}$ and $J_k^{j_k}$? Should $J_k^{j_k}$ be with respect to $x_{j_k}$?}
\begin{equation*}
\begin{aligned}
